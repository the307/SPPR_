...
from .calculate import (
    ...
    planned_balance_for_bp_vo_calc,
    planned_balance_for_bp_lodochny_calc,
    planned_balance_for_bp_tagul_calc,
    planned_balance_for_bp_vn_gtm_calc,
    planned_balance_for_bp_suzun_gtm_calc,
...
from .data_prep import (
    ...
    get_planned_balance_for_bp_vo_data,
    get_planned_balance_for_bp_lodochny_data,
    get_planned_balance_for_bp_tagul_data,
    get_planned_balance_for_bp_vn_gtm_data,
    get_planned_balance_for_bp_suzun_gtm_data,
...
def stage_planned_balance_for_bp_vo(master_df: pd.DataFrame, dates: list[pd.Timestamp]) -> pd.DataFrame:
    ...
    result = planned_balance_for_bp_vo_calc(**payload)
    master_df = update_df(master_df, {"date": last_date, **result})
    return master_df
...
def stage_planned_balance_for_bp_lodochny(master_df: pd.DataFrame, dates: list[pd.Timestamp]) -> pd.DataFrame:
    ...
    result = planned_balance_for_bp_lodochny_calc(**payload)
    master_df = update_df(master_df, {"date": last_date, **result})
    return master_df
...
def stage_planned_balance_for_bp_tagul(master_df: pd.DataFrame, dates: list[pd.Timestamp]) -> pd.DataFrame:
    ...
    result = planned_balance_for_bp_tagul_calc(**payload)
    master_df = update_df(master_df, {"date": last_date, **result})
    return master_df
...
def stage_planned_balance_for_bp_vn_gtm(master_df: pd.DataFrame, dates: list[pd.Timestamp], prev_month) -> pd.DataFrame:
    ...
    result = planned_balance_for_bp_vn_gtm_calc(**payload)
    master_df = update_df(master_df, {"date": last_date, **result})
    return master_df
...
def stage_planned_balance_for_bp_suzun_gtm(master_df: pd.DataFrame, dates: list[pd.Timestamp], prev_month) -> pd.DataFrame:
    ...
    result = planned_balance_for_bp_suzun_gtm_calc(**payload)
    master_df = update_df(master_df, {"date": last_date, **result})
    return master_df
...
def stage_planned_balance_for_bp_vo_gtm(master_df: pd.DataFrame, dates: list[pd.Timestamp], prev_month) -> pd.DataFrame:
    ...
    result = planned_balance_for_bp_vo_gtm_calc(**payload)
    master_df = update_df(master_df, {"date": last_date, **result})
    return master_df
...
def stage_planned_balance_for_bp_lodochny_gtm(master_df: pd.DataFrame, dates: list[pd.Timestamp], prev_month) -> pd.DataFrame:
    ...
    result = planned_balance_for_bp_lodochny_gtm_calc(**payload)
    master_df = update_df(master_df, {"date": last_date, **result})
    return master_df
...
def stage_planned_balance_for_bp_tagul_gtm(master_df: pd.DataFrame, dates: list[pd.Timestamp], prev_month) -> pd.DataFrame:
    ...
    result = planned_balance_for_bp_tagul_gtm_calc(**payload)
    master_df = update_df(master_df, {"date": last_date, **result})
    return master_df
...
def calculate_json_data(input_data):
    ...
    #input_data = load_input_json("data_input.json")
    
    try:

        master_df = build_master_df(input_data)
